            ...
            <table className="w-full">
              ...
              <tbody className="divide-y">
                {filteredSummary.map((student, idx) => {
                  ...
                  const studentAttendance = events.map(event => {
                    const attendance = eventAttendance[event.id]?.attendance?.find(
                      a => a.studentId === student.studentId
                    );
                    const fine = eventAttendance[event.id]?.fines?.find(
                      f => f.studentId === student.studentId
                    );
                    return { event, attendance, fine };
                  }).filter(item => item.attendance || item.fine);
                  ...
                  return (
                    <React.Fragment key={idx}>
                      ...
                      {isExpanded && studentAttendance.length > 0 && (
                        <tr>
                                ...
                                <table className="w-full">
                                  <thead className="bg-gray-100 border-b">
                                    <tr>
                                      ...
                                      <th className="px-3 py-2 text-right text-xs font-semibold text-gray-700">Fine Amount</th>
                                      <th className="px-3 py-2 text-center text-xs font-semibold text-gray-700">Fine Status</th>
                                    </tr>
                                  </thead>
                                  <tbody className="divide-y">
                                    {studentAttendance.map(({ event, attendance, fine }, detailIdx) => (
